...
utils.makeUa = function() {
    return pkg.name + '/v' + pkg.version + ' (' + process.platform + '; ' + process.arch + ') ' + 'Node/' + process.version;
};
...
utils.genReqOpts = function(thisArg, method, remotePath, length, custom) {
    ...
    remotePath = path.join('/', thisArg._conf.bucket, remotePath).replace(/[\/\\]+/g, '/');
